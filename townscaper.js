...
async function run() {
  ...
  nOf(addCount, () => cornerHeightColour.getMap(corners.any()).set(heights.any(), colours.any()));
  ...
  console.log(cornerCount, 'corners', heightCount, 'heights', colourCount, 'colours',
    addCount, 'blocks', blockCount, 'unique', Math.floor(100 * density), '% occupied');
  await writeFile(settingsFile, settingsText());
  await writeFile(saveFile, saveText(cornerHeightColour));
  console.log(saveFile, 'written');
}
...
async function getGrid(file) {
  const gridText = await readFile(file);
...
function cornerXml({ x, y }, count) {
  return `    <C><x>${x}</x><y>${y}</y><count>${count}</count></C>\n`;
}

function voxelXml(colour, height) { return `    <V><t>${colour}</t><h>${height}</h></V>\n`; }

function rand(min, max, power = 1) {
  return Math.floor(min + (max - min + 1) * Math.random() ** power);
}
...
Map.prototype.totalSize = function totalSize() {
  return [...this.values()].map((m) => m.size).sum();
};
